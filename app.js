// Python 30-Day Bootcamp (Friendly Mentor) — bilingual + tracking + quizzes + Pyodide runner
...
const i18n = {
  en: {
    ...
    mentorTipTitle: "Mentor tip", mentorTipBody: "Don’t rush. One small win per day is enough.",
    markDone: "Mark done", done: "Done",
    concept: "Concept", tryIt: "Try it (run Python)", run: "Run",
    resetCode: "Reset code", copy: "Copy", output: "Output",
  ...
  zh: {
    ...
    mentorTipTitle: "教练提示", mentorTipBody: "别着急。每天一个小胜利就够了。",
    markDone: "标记完成", done: "已完成",
    concept: "讲解", tryIt: "动手练习（运行 Python）", run: "运行",
    resetCode: "重置代码", copy: "复制", output: "输出",
...
const STATE_KEY = "pybootcamp_state_v1";
const LANG_KEY  = "pybootcamp_lang_v1";
...
function loadState(){
  try{
    return JSON.parse(localStorage.getItem(STATE_KEY)) || {
      started_on:null, completed:{}, hw_checks:{}, notes:{}, code:{}, quiz:{}
    };
  }catch{
...
function mdToHtml(md){
  ...
  md = md.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, _l, code)=>`<pre><code>${esc(code)}</code></pre>`);
...
function renderModules(){
  const el=$("#moduleList"); el.innerHTML="";
  course.modules.forEach(m=>{
    const box=document.createElement("div"); box.className="moduleItem";
    const t=document.createElement("div"); t.className="moduleTitle"; t.textContent=m.title[lang];
    const d=document.createElement("div"); d.className="moduleDays"; d.textContent=`Day ${m.days[0]}–${m.days[m.days.length-1]}`;
    box.appendChild(t); box.appendChild(d); el.appendChild(box);
  });
}
...
function renderDayList(){
  ...
  for(let n=1;n<=30;n++){
    ...
    const item=document.createElement("div"); item.className="dayItem"; item.dataset.day=String(n);
    const left=document.createElement("div"); left.className="dayLeft";
    const name=document.createElement("div"); name.className="dayName"; name.textContent=`${n}. ${day.title[lang]}`;
    const desc=document.createElement("div"); desc.className="dayDesc"; desc.textContent=day.time[lang];
    left.appendChild(name); left.appendChild(desc);
    const badge=document.createElement("div"); badge.className="badge";
    const done=!!st.completed[String(n)];
    if(done){ badge.textContent=i18n[lang].done; badge.classList.add("done"); }
    else if(n===todayN){ badge.textContent=i18n[lang].todayBadge; badge.classList.add("today"); }
    else { badge.textContent=`Day ${n}`; }
...
function renderQuiz(n){
  const day = course.days[n-1];
  const st = loadState();
  const wrap=document.createElement("div"); wrap.className="quizBox";
  const q=document.createElement("div"); q.className="quizQ"; q.textContent=day.quiz.q[lang];
  wrap.appendChild(q);
  const saved=st.quiz[String(n)];
  const result=document.createElement("div"); result.className="quizResult";
  day.quiz.options[lang].forEach((opt, idx)=>{
    const row=document.createElement("div"); row.className="quizOpt"; row.textContent=opt;
    if(saved?.answered){
  ...
  $("#quiz").innerHTML=""; $("#quiz").appendChild(wrap);
}
...
function openDay(n){
  ...
  $("#dayTitle").textContent = `${n}. ${day.title[lang]}`;
  ...
  const hwWrap=document.createElement("div"); hwWrap.className="hwList";
  hw.forEach((t, idx)=>{
    const row=document.createElement("label"); row.className="hwItem";
  ...
  $("#homework").innerHTML=""; $("#homework").appendChild(hwWrap);
...
function startToday(){
  ...
  renderDashboard(); renderDayList(); goToday();
}
...
function resetAll(){
  ...
  renderDashboard(); renderDayList(); openDay(1);
}
